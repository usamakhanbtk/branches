...
import HeaderP from './HeaderPatient';
...
const Edit = () => {
    ...
    useEffect(()=>{
        setUser(user => ({ ...user, name: storedObject[0].name }));
            setUser(user => ({ ...user, id: storedObject[0].p_id }));
            ...
            setUser(user => ({ ...user, gender: storedObject[0].gender }));
             setUser(user => ({ ...user, email: storedObject[0].email }));
              setUser(user => ({ ...user, password: storedObject[0].password }));
               setUser(user => ({ ...user, balance: storedObject[0].balance }));
    },[])
    const Cities = ['Faislabad','Lahore','Multan','Karachi','Queeta','Sialkot','Islamabad'];
    const handleAdd = () =>{
        Axios.put("http://localhost:5001/updatePatient",{
            id:user.id,
            name:user.name,
            gender:user.gender,
            email:user.email,
